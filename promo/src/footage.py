"""Mixkit 무료 클립을 내려받아 오프닝·드롭 A 배경용 프레임(30fps)으로 잘라 둔다.
...
for fmt, (W, H) in {'h': (1920, 1080), 'v': (1080, 1920)}.items():
    ...
    for key, c in cfg.items():
        ...
        N = math.ceil(c.get('dur', 0.625) * 30) + 1
        for old in os.listdir(out):
            if old.endswith('.jpg'): os.remove(os.path.join(out, old))
        if fmt == 'h':
        ...
        vf = (f'setpts=PTS/{sp},fps=30,{geo},'
              f'eq=contrast=1.05:saturation=0.82:gamma={1/gain:.3f},colorbalance=bs=0.05:bm=0.02:rs=-0.02,'
              f'unsharp=5:5:0.6')
        subprocess.run([FF, '-y', '-loglevel', 'error', '-ss', str(c['in']), '-i', os.path.join(src_dir, f"{c['id']}-720.mp4"),
                        '-vf', vf, '-frames:v', str(N), '-q:v', '3', os.path.join(out, '%03d.jpg')], check=True)
